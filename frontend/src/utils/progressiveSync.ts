...
import { offlineStorage, OfflineAnalysis, OfflineAction, SyncStatus } from './offlineStorage'
    ...
    try {
      ...
      for (const item of batch.items) {
        ...
        try {
          ...
          // Update item status
          if ('farmerId' in item) {
            await offlineStorage.updateUploadStatus(item.id, 'failed', error.message)
          }
        }
